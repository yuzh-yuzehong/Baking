...
namespace Machine
{
    public partial class OtherDebugPage : FormEx
    {
            ...
            try
            {
                ...
                if(txt.Length > 0)
                {
                    ...
                    if(null != run)
                    {
                        ...
                        if(pltIdx > -1)
                        {
                            //if(!run.ManualSetPalletState(pltIdx , PalletStatus.NG)) {
                            //    ShowMsgBox.ShowDialog("无托盘，请先添加托盘。" , MessageType.MsgWarning);
                            //}
            ...
            try
            {
                ...
                if(txt.Length > 0)
                {
                    ...
                    if(null != run)
                    {
                        ...
                        Def.WriteLog("ResetEvent", string.Format("++++++++++++++++++++++++++++++ResetEvent {0} Success++++++++++++++++++++++++++++++", txt[1]));
                    }
                }
            }
            catch(System.Exception ex)
            {
                System.Diagnostics.Trace.WriteLine("buttonResetEvent_Click() error : " + ex.Message);
            }
        }
        #endregion
        ...
        /// <param name="e"></param>
        private void btnOKState_Click(object sender, EventArgs e)
        {
            try
            {
                string[] txt = this.comboBoxPalletModule.SelectedItem.ToString().Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
                if (txt.Length > 0) ;
                {
                    ...
                    if (null != run)
                    {
                        ...
                        if (pltIdx > -1)
                        {
                            run.ManualSetPalletState(pltIdx, PalletStatus.OK);
                            Def.WriteLog("OtherDebugPage", string.Format("{0}:置OK夹具状态，不改变电芯状态{1}", run.RunName, (pltIdx + 1)));
                        }
                        else
                        {
                            ShowMsgBox.ShowDialog("夹具索引无效，添加失败", MessageType.MsgWarning);
            ...
            {
                System.Diagnostics.Trace.WriteLine("btnOKState_Click() error : " + ex.Message);
            }
        }
    }
}
