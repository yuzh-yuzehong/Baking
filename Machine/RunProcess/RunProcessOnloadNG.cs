...
namespace Machine {
    ...
    class RunProcessOnloadNG : RunProcess {
        ...
        protected override void AutoOperation() {
            ...
            switch((AutoSteps)this.nextAutoStep) {
                case AutoSteps.Auto_WaitWorkStart: {
                    ...
                    // 有空位，发送放电池信号
                    EventStatus state = GetEvent(this, EventList.OnloadNGPlaceBattery);
                    if(!PlacePosIsFull()&&((EventStatus.Invalid==state)||(EventStatus.Finished==state))) {
                        SetEvent(this , EventList.OnloadNGPlaceBattery , EventStatus.Require);
                    }
                    // 有空位，已响应
                        ...
                        if(PlaceSenserIsSafe(true)) {
                            ...
                            SetEvent(this , EventList.OnloadNGPlaceBattery , EventStatus.Ready);
                        }
                    }
                ...
                case AutoSteps.Auto_SendToBufferPos: {
                    ...
                    if(!sendFin) {
                        ...
                        if(EventStatus.Cancel==GetEvent(this , EventList.OnloadNGPlaceBattery)) {
                            SetEvent(this , EventList.OnloadNGPlaceBattery , EventStatus.Invalid);
                        }
                ...
                case AutoSteps.Auto_SendToManualPos: {
                    ...
                    bool sendFin = false;
                    DateTime time = DateTime.Now;
                    while(InputState(IBufferPosInpos , false)) {
                        OutputAction(ONGLineAlarm , true);
                        OutputAction(ONGLineMotor , true);
                        OutputAction(ONGBufferMotor , true);
                        //延时退出
        ...
        /// </summary>
        /// <returns></returns>
        public bool GetPlacePos(int count , ref int pos) {
            if(count<=0||count>(int)ModDef.PlacePos_ALL)
                return false;
            for(int i = 0 ; i<(int)ModDef.PlacePos_ALL-count+1 ; i++) {
                bool ret =true;
                for(int j = 0 ; j<count ; j++) {
                    if(BatteryStatus.Invalid!=this.Battery[i+j].Type) {
                        ret=false;
                        break;
                    }
                }
