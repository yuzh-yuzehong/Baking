...
namespace Machine
{
    public partial class UpDataWaterPage : FormEx
    {
        ...
        {
            // 手动解绑
            string[] str = this.textBoxUnbindPlt.Text.Split(';', '；');
            bool result = MachineCtrl.GetInstance().MesUnbindPalletInfo(str[0], (str.Length > 1 ? str[1] : "02"), MesResources.Heartbeat);
            ShowMsgBox.Show("夹具解绑" + (result ? "成功" : "失败"), MessageType.MsgMessage);
        }
        ...
        private void buttonParaUpdata_Click(object sender, EventArgs e)
        {
            ...
            bool result = MachineCtrl.GetInstance().MesGetBillParameter(getParam);
            if(result)
            {
                string msg = $"工艺参数获取成功\r\n";
